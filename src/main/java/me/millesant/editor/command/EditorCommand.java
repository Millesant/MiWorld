package me.millesant.editor.command;
...
import me.millesant.editor.operation.*;

@Getter
public class EditorCommand extends Command {

    private final MiWorld plugin;

    public EditorCommand(final MiWorld plugin) {
        super("editor", "The main command for the world editor", null, new String[]{"we"});
        this.plugin = plugin;
    }
    ...
    public boolean execute(final CommandSender sender, final String commandLabel, final String[] arguments) {
        ...
        return switch (arguments[0]) {
                ...
                yield switch (arguments[1]) {
                    ...
                    case "point", "p" -> {
                        ...
                        yield switch (arguments[2]) {
                            case "first", "f" -> {
                                final var session = this.getPlugin().getSessionService().getSession(player.getName()).orElseThrow();

                                final var selection = session.getSelection();
                                ...
                                player.sendMessage(String.format("Set first point to %s", player.getPosition()));
                                yield true;
                            }
                            case "second", "s" -> {
                                final var session = this.getPlugin().getSessionService().getSession(player.getName()).orElseThrow();
                                ...
                                player.sendMessage(String.format("Set second point to %s", player.getPosition()));
                                yield true;
                            }
                            default -> {
            ...
            case "set" -> {
                ...
                if (!(selection.hasFirstPoint() && selection.hasSecondPoint())) {
                ...
                final var operation = new SetOperation(session, block);

                this.getPlugin().getWorldService().execute(player.getLevel(), operation);

                player.sendMessage(String.format("Operation '%s' executed successfully", operation.getName()));
                yield true;
            }
            case "replace" -> {
                final var session = this.getPlugin().getSessionService().getSession(player.getName()).orElseThrow();
                ...
                if (!(fromBlockArgument.matches("[0-9]+"))) {
                    player.sendMessage("Invalid block id provided");
                    yield false;
                }
                ...
                if (!(toBlockArgument.matches("[0-9]+"))) {
                    player.sendMessage("Invalid block id provided");
                    yield false;
                }
                ...
                final var operation = new ReplaceOperation(session, fromBlock, toBlock);
            ...
            case "undo" -> {
                ...
                final var operation = new UndoOperation(session);
            ...
            case "redo" -> {
                ...
                final var operation = new RedoOperation(session);
            ...
            case "copy" -> {
                ...
                final var operation = new CopyOperation(session);
            ...
            case "paste" -> {
                ...
                final var operation = new PasteOperation(session);
            ...
            case "cut" -> {
                ...
                final var operation = new CutOperation(session);
            ...
            case "rotate" -> {
                ...
                if (!(angleArgument.matches("90|180|270"))) {
                ...
                final var operation = new RotateOperation(session, angle);
            ...
            case "flip" -> {
                ...
                if (!(directionArgument.matches("horizontal|vertical"))) {
                ...
                final var operation = new FlipOperation(session, directionArgument);
            ...
            case "fill" -> {
                ...
                final var operation = new FillOperation(session, block);
            ...
            case "outline" -> {
                ...
                final var operation = new OutlineOperation(session, block);
            ...
            case "move" -> {
                ...
                if (!(xArgument.matches("-?[0-9]+") && yArgument.matches("-?[0-9]+") && zArgument.matches("-?[0-9]+"))) {
                ...
                final var operation = new MoveOperation(session, x, y, z);
            ...
            case "scale" -> {
                ...
                if (!(factorArgument.matches("[0-9]+(\\.[0-9]+)?"))) {
                ...
                final var operation = new ScaleOperation(session, factor);
